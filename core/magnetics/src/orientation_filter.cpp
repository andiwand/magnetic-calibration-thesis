...
namespace {
void MadgwickAHRSupdateIMU(float dt, float beta, float gx, float gy, float gz, float ax, float ay, float az, float &q0, float &q1, float &q2, float &q3);
}
...
void MadgwickImu::iterate() {
  ...
  for (std::size_t i = 0; i < acc.size(); ++i) {
    if (std::isnan(gyr[i].data.x) || std::isnan(gyr[i].data.y) || std::isnan(gyr[i].data.z) ||
        std::isnan(acc[i].data.x) || std::isnan(acc[i].data.y) || std::isnan(acc[i].data.z)) {
      continue;
    }
...
void MadgwickAHRSupdateIMU(float dt, float beta, float gx, float gy, float gz, float ax, float ay, float az, float &q0, float &q1, float &q2, float &q3) {
  ...
  if(!((ax == 0.0f) && (ay == 0.0f) && (az == 0.0f))) {
    ...
    recipNorm = std::pow(s0 * s0 + s1 * s1 + s2 * s2 + s3 * s3, -0.5); // normalise step magnitude
    s0 *= recipNorm;
    s1 *= recipNorm;
    s2 *= recipNorm;
    s3 *= recipNorm;
