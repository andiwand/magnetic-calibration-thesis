...
public:
  ...
  std::size_t size() const { return std::end(m_buffer) - m_buffer_begin; }
  ...
  Series<T> pop_until(const double time) const {
    ...
    const auto buffer_end =
        std::begin(m_buffer) + result.m_data_end - m_buffer.data();
    m_buffer_begin = buffer_end;
    return result;
  }

  Series<T> front(const double time_end) const {
    const auto buffer_being =
        m_buffer.data() + m_buffer_begin - std::begin(m_buffer);
    const auto buffer_end =
        m_buffer.data() + find_next(time_end) - std::end(m_buffer);
    return {m_time_begin, time_end, buffer_being, buffer_end};
  }

  void clear() {
    m_buffer.clear();
    m_buffer_begin = std::begin(m_buffer);
  }
  ...
  void clear_front(const Series<T> &series) {
    m_buffer.erase(std::begin(m_buffer),
                   m_buffer_begin + series.m_data_end - m_buffer.data());
    m_buffer_begin = std::begin(m_buffer);
    m_time_begin = series.m_time_end;
  }
  ...
  void rotate() {
    m_buffer.erase(std::begin(m_buffer), m_buffer_begin);
    m_buffer_begin = std::begin(m_buffer);
  }
...
private:
  ...
  typename std::vector<T>::iterator m_buffer_begin{std::begin(m_buffer)};
  ...
  auto find_next(const double time) const {
    if ((time < m_time_begin) || (time > m_time_end))
      throw; // TODO
    return std::find_if(m_buffer_begin, std::end(m_buffer),
